...
aws_request_form(Method, Protocol, Host, Port, Path, Form, Headers, Config) ->
	...
	URL = case Port of
			undefined ->
				  [UProtocol, Host, Path];
			_ ->
				  [UProtocol, Host, $:, port_to_str(Port), Path]
		  end,
...
%% TODO remove use of process dictionary. An ETS table is better than mucking around in the plumbing...
default_config() ->
